...
cat("
    ...
    # Log-scale intercept and slope parameters
    intercept ~ dnorm(0,0.001)
    slope ~ dnorm(0,0.001)
    
    # Route random effects
    route_sd ~ dunif(0,5)
    route_tau <- pow(route_sd,-2)
    for (r in 1:n_route){
      route_effect[r] ~ dnorm(0,route_tau)
    }
    ...
    for (y in 1:n_year){
      year_effect[y] ~ dnorm(0,year_tau)
    }
    
    # Residual noise
    error_sd ~ dunif(0,5)
    error_tau <- pow(error_sd,-2)
    ...
    for (i in 1:n_obs){
      ...
      mu[i] <- intercept + slope * (year[i] - 1) + route_effect[route[i]] + year_effect[year[i]]
      log_lambda[i] ~ dnorm(mu[i],error_tau)
      lambda[i] <- exp(log_lambda[i])
      count[i] ~ dpois(lambda[i])
    }
  
    # ------------------------------
    # Derived quantities (annual indices and measures of trend)
    # ------------------------------
    
    # Annual indices of relative abundance
    for (y in 1:n_year){
    
      # Based on year effects
      index[y] <- exp(intercept + slope * (y-1) + 0.5*pow(route_sd,2) + year_effect[y])
      
    }
    
    # Estimates of mean percent change per year
    trend <- 100 * ( pow( index[n_year]/index[1], 1/(n_year-1) )-1)
...
for (sp in species_summary$Species){
  ...
  jags_data = list(n_obs = nrow(sp_dat),
                   n_year = max(sp_dat$Year_Number),
                   n_route = max(sp_dat$Route_Number),
                   year = sp_dat$Year_Number,
                   route = sp_dat$Route_Number,
                   count = sp_dat$count)
  ...
  parameters.to.save = c("index","trend")
  out <- jags(data = jags_data,
              ...
              n.burnin = 10000,
              n.thin = 5,
              model.file = "model_abund.jags",
              n.chains = 3,
              parallel = TRUE)

  #save(out, file = paste0("../output/",sp,"_out_abund.Rdata"))
  
  # ----------------------------------------
  # Extract output and plot
  # ----------------------------------------
  
  #load(file = paste0("../output/",sp,"_out_abund.Rdata"))
  
  # ----------------------------------------
  # Extract and plot annual indices
  # ----------------------------------------
  
  annual_index = out$sims.list$index %>%
    reshape2::melt() %>%
  ...
  annual_index_plot = ggplot()+
    #geom_errorbar(data = annual_index, aes(x = Year, ymin = q05, ymax = q95, y = q50), width = 0)+
    #geom_point(data = annual_index, aes(x = Year,y = q50))+
    
    geom_ribbon(data = annual_index, aes(x = Year, ymin = q05, ymax = q95, y = q50), 
                fill = "dodgerblue", alpha = 0.2)+
    geom_line(data = annual_index, aes(x = Year,y = q50), col = "dodgerblue")+
    
    #geom_point(data = subset(annual_summary, Species == sp),aes(x = Year, y = prop_obs),col = "dodgerblue")+
    ggtitle("Annual relative abundance")+
    ylab("Population Index")+
    xlab("Year")
  
  # print(annual_index_plot)
  ...
  species_plot <- ggarrange(annual_index_plot, trend_plot,
  ...
  png(paste0("../output/figures/",sp,"_abund.png"),width=10,height=6, units = "in", res = 500)
